...
async function problemA() {
  ...
  // AsyncAwait version
  async function readFileSyn(filename) {
    blue(await promisifiedReadFile(filename));
  }

  await Promise.all([
    readFileSyn("poem-two/stanza-01.txt"),
    readFileSyn("poem-two/stanza-02.txt"),
  ]);
...
async function problemB() {
  ...
  const arrPromesas = filenames.map((fn) => readFileSyn(fn));
