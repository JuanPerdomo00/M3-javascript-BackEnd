...
function problemB() {
  ...
  // promise version
  // ???
  const listPromest = filenames.map((file) =>
    promisifiedReadFile(file).then(blue)
  );

  Promise.all(listPromest).then(() => {
    console.log("done");
  });
}
  ...
  function promisifiedWriteFile(filename, str) {
    ...
    return new Promise((resolve, reject) => {
      fs.writeFile(filename, str, "utf-8", (err) => {
        if (err) reject(err);
        resolve("Good for you jakepys :)");
      });
    });
  }

  promisifiedWriteFile("./hola.txt", `Escrito desde la funcion. :)`);
}
